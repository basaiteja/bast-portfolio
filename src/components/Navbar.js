        ...
        <Box component={'nav'} width={'100%'} position={singlePage ? 'fixed' : 'relative'} className={darkMode? Style.dark : Style.light}>
            <Box component={'ul'} display={'flex'} justifyContent={'center'} alignItems={'center'}
                ...
                {links.map((link, index) => (
                    <Box key={index} component={'li'} id={'navli'} className={(link.active === active && !link.type) && Style.active}
                ...
                {/* <li>
                    <Toggler darkMode={darkMode} handleClick={handleClick} /> //dark /light mode
                </li> */ }   
            </Box>
        </Box>
    )
